...
function Health.Create()
    ...
    s.Text = Utils.NewDrawing("Text",   {Size=10, Color=Color3.new(1,1,1), Outline=false, Center=true})
    return s
end
    ...
    if cfg.ShowText then
        ...
        if top then
            ...
            if on then
                ...
                self.Text.Position = Vector2.new(sp.X, sp.Y - 30)
...
function Health:Hide()   self.Bg.Visible=false; self.Bar.Visible=false; self.Text.Visible=false end
function Health:Remove() Utils.Kill(self.Bg); Utils.Kill(self.Bar); Utils.Kill(self.Text) end
return Health
