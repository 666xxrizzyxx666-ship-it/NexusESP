...
local Utils, Config
local Mods = {}   -- Box, CornerBox, Skeleton, Tracers, Health, Name, Distance
local entities = {}
...
function ESP.Init(deps)
    ...
    Mods.Health=deps.Health; Mods.Name=deps.Name; Mods.Distance=deps.Distance
    ESP._modules = Mods   -- expose for Preview
...
local function create(player)
    ...
    entities[player]={
        box=safe(Mods.Box), cornerBox=safe(Mods.CornerBox),
        skeleton=safe(Mods.Skeleton), tracers=safe(Mods.Tracers),
        health=safe(Mods.Health), name=safe(Mods.Name, player),
        distance=safe(Mods.Distance), disabled=false,
    }
end
...
local function update(player, ent)
    ...
    if Utils.GetDistance(root.Position)>(cfg.Distance and cfg.Distance.MaxDist or 800) then hideAll(ent); return end
...
local function onRender()
    ...
    frame=frame+1
    for p,e in pairs(entities) do pcall(update,p,e) end
end

function ESP.Enable()
    if enabled then return end; enabled=true
    for _,p in ipairs(Players:GetPlayers()) do
        if p~=Players.LocalPlayer then create(p) end
    end
