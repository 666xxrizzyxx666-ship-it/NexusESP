...
function ESP.Init(deps)
    ...
    -- SetDependencies avec vérification nil
...
local function safeCreate(module, player)
    if module and module.Create then
        local ok, r = pcall(module.Create, player)
        if ok then return r end
    end
    -- Fallback vide si module absent
...
local function createEntity(player)
    ...
    entities[player] = {
        box       = safeCreate(Box,       player),
        cornerBox = safeCreate(CornerBox, player),
        skeleton  = safeCreate(Skeleton,  player),
        tracers   = safeCreate(Tracers,   player),
        health    = safeCreate(Health,    player),
        name      = safeCreate(NameMod,   player),
        distance  = safeCreate(Distance,  player),
        disabled  = false,
    }
end
...
local function updateEntity(player, ent)
    ...
    local fullUpdate = not cfg.PerformanceMode or (frameCount%2==0)
    ...
    if ent.box and ent.box.Frame then
